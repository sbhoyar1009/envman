 ...
 */
export async function initCommand(projectNameArg?: string): Promise<void> {
  try {
    ...
    const projectName =
      projectNameArg || (await promptProjectName(defaultName))
    ...
    const shouldScan = await promptConfirm(
      'Scan codebase for existing environment variables?'
    )
