...
export class ConfigManager {
  ...
  private userConfig: Conf<UserCredentials>
  private envmanDir: string

  constructor() {
    this.envmanDir = path.join(process.cwd(), '.envman')
    this.projectConfigPath = path.join(this.envmanDir, 'config.json')
    this.userConfig = new Conf<UserCredentials>({
      projectName: 'envman',
   ...
   */
  async requirePermission(permission: string, operation: string = 'perform this operation'): Promise<void> {
    if (!(await this.hasPermission(permission))) {
   ...
   */
  async requireAnyPermission(permissions: string[], operation: string = 'perform this operation'): Promise<void> {
