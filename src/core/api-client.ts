...
export class APIClient {
  ...
  constructor(baseURL: string = 'https://api.envman.dev', token?: string) {
  ...
  async pullVariables(
    ...
    try {
      ...
      const encryptedVariables: EncryptedVariable[] = (variables || []).map(v => ({
        key: v.key,
   ...
   */
  async getTeamConfig(project: string): Promise<TeamConfig> {
    try {
      // Get project
      const { data: projectData, error: projectError } = await this.supabase
        .from('projects')
   ...
   */
  async updateTeamConfig(project: string, teamConfig: TeamConfig): Promise<APIResponse<TeamConfig>> {
    try {
      // Get project
      const { data: projectData, error: projectError } = await this.supabase
        .from('projects')
   ...
   */
  async inviteTeamMember(project: string, email: string, role: string): Promise<APIResponse<{email: string, role: string, status: string}>> {
    try {
      // Get project
      const { data: projectData, error: projectError } = await this.supabase
        .from('projects')
   ...
   * Remove team member
   */
  async removeTeamMember(project: string, email: string): Promise<APIResponse<{email: string}>> {
    try {
      // Get project
      const { data: projectData, error: projectError } = await this.supabase
        .from('projects')
   ...
   */
  async getUserPermissions(project: string, email: string): Promise<string[]> {
    try {
   ...
   */
  async getPendingInvites(): Promise<Array<{project: string, role: string, invitedBy: string, invitedAt: string}>> {
    try {
      const { data: { user }, error: userError } = await this.supabase.auth.getUser()
      if (userError || !user) {
        throw new Error('Not authenticated')
      ...
      return (invites || []).map(invite => ({
        project: (invite.projects as any).name,
        role: invite.role,
   ...
   */
  async acceptInvite(project: string): Promise<APIResponse<{project: string, role: string}>> {
    try {
      const { data: { user }, error: userError } = await this.supabase.auth.getUser()
      if (userError || !user) {
        throw new Error('Not authenticated')
   ...
   */
  async joinProjectByHash(hash: string): Promise<APIResponse<{project: string, role: string}>> {
    try {
      const { data: { user }, error: userError } = await this.supabase.auth.getUser()
      if (userError || !user) {
        throw new Error('Not authenticated')
   ...
   */
  async createProject(projectName: string): Promise<APIResponse<{project: string, role: string}>> {
    try {
      const { data: { user }, error: userError } = await this.supabase.auth.getUser()
      if (userError || !user) {
        throw new Error('Not authenticated')
   ...
   */
  async getProjectHash(projectName: string): Promise<string> {
    try {
   ...
   */
  private generateInviteHash(): string {
   ...
   */
  async diagnoseSchema(): Promise<void> {
    try {
