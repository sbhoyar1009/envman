 ...
 */
function displayUnusedOnly(result) {
    logger.blank();
 ...
 */
function displayMissingOnly(result) {
    logger.blank();
 ...
 */
async function exportResults(result, outputPath) {
    ...
    const exportData = {
        ...
        detailed_usage: Object.fromEntries(Array.from(result.usedVars.entries()).map(([key, usages]) => [
            key,
            usages.map((u) => ({
                file: u.file,
                line: u.line,
