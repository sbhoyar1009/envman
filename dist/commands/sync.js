 ...
 */
async function startWatchMode(environment, envFile, options) {
    ...
    watcher.on('env-changed', async (changes) => {
        if (pullOnly)
            return; // Skip if pull-only mode
 ...
 */
async function performOneTimeSync(environment, envFile, options) {
 ...
 */
async function performPush(environment, envFile, reason) {
    ...
    try {
        ...
        // Push to API
        const response = await apiClient.pushVariables(configManager.loadProject().projectName, environment, encryptedVars);
        if (!response.success) {
 ...
 * Pull remote changes
 */
async function performPull(environment, envFile) {
    const spinner = ora('Checking for remote changes...').start();
    try {
        // Get remote variables
        const response = await apiClient.pullVariables(configManager.loadProject().projectName, environment);
        ...
        const hasChanges = Object.keys(remoteVars).some(key => !(key in localVars) || localVars[key] !== remoteVars[key]);
        if (!hasChanges) {
 ...
 */
async function checkRemoteChanges(environment, envFile) {
    try {
        const response = await apiClient.pullVariables(configManager.loadProject().projectName, environment);
        if (!response.success || !response.data) {
        ...
        if (hasNewOrChanged) {
            ...
            const modified = Object.keys(remoteVars).filter(key => key in localVars && localVars[key] !== remoteVars[key]);
            if (added.length > 0) {
